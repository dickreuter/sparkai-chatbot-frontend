...
function CustomEditor({ editorState, setEditorState, appendResponse, disabled }) {
    ...
    const applyBoldToHeadings = useCallback((editorState) => {
        ...
        return modified ? EditorState.push(editorState, newContentState, 'change-inline-style') : editorState;
    }, []);
    ...
    useEffect(() => {
        if (appendResponse) {
            const newEditorState = appendResponseAndStyle(localEditorState, appendResponse);
            setLocalEditorState(newEditorState);
            setEditorState(newEditorState);
            debouncedUpdateSharedState(newEditorState);
        }
    }, [appendResponse, localEditorState, setEditorState, debouncedUpdateSharedState]);
