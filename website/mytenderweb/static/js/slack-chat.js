...
(function($) {
            ...
            `;
                    ...
                    $('#slackChatTrigger').on('click', function(e) {
                        ...
                        if ($chatBox.is(':visible')) {
                            ...
                            $('.slack-message-box').height($chatBox.height() - $('.desc').height() - $('.send-area').height() - parseInt(window.slackChat._options.heightOffset));

                            function querySlackChannel() {
                                if ($chatBox.hasClass('open') || window.slackChat._options.privateChannel) {
                ...
                querySlack: function($elem) {
                        ...
                        methods.createChannel($elem, function(channel) {
                                                        ...
                                                        if (resp.ok && resp.messages.length) {
                                                            ...
                                                            for (const message of resp.messages) {
                                                                if (message.subtype === 'bot_message' && message.text) {
                                                                    ...
                                                                    const messageText = methods.formatMessage(message.text.trim());

                                                                    html += `
                                        <div class='message-item'>
                                            ${userImg ? `<div class='userImg'><img src='${userImg}' /></div>` : 
                                              (options.defaultUserImg ? `<div class='userImg'><img src='${options.defaultUserImg}' /></div>` : '')}
                                            <div class='msgBox'>
                            ...
                            if(repliesExist > 0 && !window.slackChat._options.isOpen && window.slackChat._options.badgeElement) {
        ...
        postMessage: function($elem) {
            ...
            const attachment = {
                ...
                fields: options.userId ? [{ title: "ID", value: options.userId, short: true }] : []
            };
                    ...
                    for(const user of userList) {
                        ...
                        if(presenceResp.ok) {
                            if(presenceResp.presence === 'active') {
                                ...
                                if(options.disableIfAway && options.elementToDisable) options.elementToDisable.show();
                                active = true;
                                break;
                            }
                        }
